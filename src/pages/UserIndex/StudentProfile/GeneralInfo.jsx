...
const GeneralInfo = () => {
  ...
  const { user, token } = useContext(AuthContext); // Obtener el usuario y token del contexto
  ...
  useEffect(() => {
    if (user && user.user_id) {
      axios
        ...
        .catch((err) => {
          console.error("Error al obtener datos:", err.response?.data || err.message); // Debug
      ...
      >
        <Container fluid className="user-index" style={{ marginTop: headerHeight }}>
          <Row>
            ...
            <Col md={9}>
              {isLoading ? (
                <p>{t('loading')}</p> // Asegúrate de tener 'loading' en tus traducciones
              ) : error ? (
